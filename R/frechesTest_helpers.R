...
.project_to_laplacian_space_osqp <- function(target_matrix_B,
  ...
  q_osqp_target <- -as.vector(target_matrix_B)
  model_to_solve <- NULL
  
  # Determine if the existing cache is valid for the current request (M_nodes and directed status)
  # The actual model building with correct constraints happens if cache is invalid.
  ...
  # This helper now assumes osqp_model_cache IS the model object if valid,
  # otherwise it builds a new one. The calling function (frechesTest)
  # handles the logic of *storing* this newly built model back into frechet_optns if needed.
  ...
  if (rebuild_this_call) {
    ...
    # Using lists to build sparse matrix components efficiently
    ...
    # 1. Symmetry Constraints (L_rc - L_cr = 0) --- ONLY IF UNDIRECTED
    if (!is_directed && num_constraints_sym > 0) {
      for (c_node in 2:M_nodes) {
        for (r_node in 1:(c_node - 1)) {
          current_max_row <- current_max_row + 1
          idx_rc <- (c_node - 1) * M_nodes + r_node
          idx_cr <- (r_node - 1) * M_nodes + c_node
          row_indices <- c(row_indices, current_max_row, current_max_row)
          col_indices <- c(col_indices, idx_rc, idx_cr)
          values      <- c(values, 1, -1)
          l_bounds_list <- c(l_bounds_list, 0)
          u_bounds_list <- c(u_bounds_list, 0)
        }
      }
    }
    
    # 2. Zero Row Sums (sum_j L_ij = 0 for all i)
    for (r_node in 1:M_nodes) {
      ...
      values      <- c(values, rep(1, M_nodes))
      l_bounds_list <- c(l_bounds_list, 0)
      u_bounds_list <- c(u_bounds_list, 0)
    }
    
    # 3. Off-diagonal Bounds (-W <= L_ij <= 0 for i != j)
    if (num_constraints_offdiag_bounds > 0) {
      if (!is_directed) { # For UNDIRECTED, only need to constrain upper (or lower) triangle
        for (c_node in 2:M_nodes) {
          for (r_node in 1:(c_node - 1)) {
            current_max_row <- current_max_row + 1
            idx_rc <- (c_node - 1) * M_nodes + r_node
            ...
            values      <- c(values, 1)
            l_bounds_list <- c(l_bounds_list, -W_upper_edge_abs_val)
            u_bounds_list <- c(u_bounds_list, 0)
          }
        }
        ...
        for(r_constr_node in 1:M_nodes) {
          for (c_constr_node in 1:M_nodes) {
            if (r_constr_node == c_constr_node) next
            current_max_row <- current_max_row + 1
            idx_rc <- (c_constr_node - 1) * M_nodes + r_constr_node
            row_indices <- c(row_indices, current_max_row)
            col_indices <- c(col_indices, idx_rc)
            values      <- c(values, 1)
            l_bounds_list <- c(l_bounds_list, -W_upper_edge_abs_val)
    ...
    if (num_total_constraints > 0) {
      const_matrix_A_sparse <- Matrix::sparseMatrix(i = row_indices, j = col_indices, x = values,
                                                    dims = c(current_max_row, M_nodes^2))
      l_bounds_vec <- l_bounds_list
      u_bounds_vec <- u_bounds_list
    } else { # Handles M_nodes = 1 if no constraints were added (only if row sum was not added)
      # This case should be rare if row sums are always added.
      # For M_nodes = 1, only L11=0 due to row sum, so num_total_constraints is 1.
      const_matrix_A_sparse <- Matrix::Matrix(0, nrow = 0, ncol = M_nodes^2, sparse = TRUE)
      l_bounds_vec <- numeric(0)
      u_bounds_vec <- numeric(0)
    }
    if (M_nodes == 1 && num_total_constraints == 0) { # If somehow no constraints were added for M_nodes=1
    ...
    current_osqp_settings <- osqp_settings %||% osqp::osqpSettings(verbose = FALSE, eps_abs = 1e-8, eps_rel = 1e-8, max_iter = 20000, polish = TRUE)
    model_to_solve <- osqp::osqp(P = P_osqp_sparse, q = q_osqp_target, # q_osqp_target is based on current data
                                 A = const_matrix_A_sparse, l = l_bounds_vec, u = u_bounds_vec,
                                 pars = current_osqp_settings)
    # The calling function (frechesTest) should update its cache with:
    # list(model = model_to_solve, M_nodes = M_nodes, directed_status = is_directed, cache_id = new_cache_id)
  } else { # Cache is valid and model_to_solve is already set
    model_to_solve$Update(q = q_osqp_target)
  }
  
  solution <- model_to_solve$Solve()
  
  if (solution$info$status_val != 1 && solution$info$status_val != 2 ) {
    ...
    projected_L_matrix <- target_matrix_B # Start with the original target
    if (!is_directed) { # Symmetrize only if undirected graph expected
      projected_L_matrix <- (target_matrix_B + t(target_matrix_B)) / 2
    }
    
    for(r_idx in 1:M_nodes) { # Apply off-diagonal constraints
    ...
    diag(projected_L_matrix) <- 0 # Temporarily zero out diagonal for sum of off-diagonals
    diag(projected_L_matrix) <- -rowSums(projected_L_matrix) # L_ii = -sum_{j!=i} L_ij
    
    # For undirected, re-symmetrize if off-diagonal constraints might have broken it.
    # And ensure diagonal is sum of new off-diagonals.
    if (!is_directed) {
      projected_L_matrix <- (projected_L_matrix + t(projected_L_matrix)) / 2
      diag(projected_L_matrix) <- 0
  ...
  projected_L_matrix <- matrix(projected_L_vector, nrow = M_nodes, ncol = M_nodes, byrow = FALSE)
...
.get_quantiles_for_obs_jump_test <- function(obs, qSup_target, den_opts) {
  # This function will be called less for its expensive branches due to pre-computation
  if (is.list(obs) && !is.null(obs$type) && obs$type == "quantile" && !is.null(obs$q) && !is.null(obs$qSup)) {
    # If obs is already a pre-computed quantile object
    if (length(obs$qSup) == length(qSup_target) && all(abs(obs$qSup - qSup_target) < 1e-9)) {
      return(obs$q) # Already on the target qSup grid
    } else {
      # Interpolate if on a different qSup grid (should ideally be avoided by consistent qSup)
    ...
    # Raw numeric data: perform density estimation
    ...
    # Histogram object: perform density estimation from histogram
    current_den_opts <- den_opts %||% list()
    if (!is.null(current_den_opts$kernelDen)) { names(current_den_opts)[names(current_den_opts) == "kernelDen"] <- "kernel" }
    if (!is.null(current_den_opts$bwDen)) { names(current_den_opts)[names(current_den_opts) == "bwDen"] <- "userBwMu" }
    if (!is.null(current_den_opts$ndSup)) { names(current_den_opts)[names(current_den_opts) == "ndSup"] <- "nRegGrid" }
...
# .calculate_one_sided_locpoly_weights (original function - no changes needed for this optimization)
.calculate_one_sided_locpoly_weights <- function(x_obs, c_val, h_val, K_fun, side, n) {
  ...
  if (length(active_indices_side) < 2) {
    warning(paste("Too few points on side", side, "for local polynomial regression (need at least 2). Weights will be zero."))
  ...
  if (abs(sigma_0_sq_hat) < 1e-20) { 
    warning(paste("sigma_0_sq_hat is close to zero for side", side, "(value:", sigma_0_sq_hat, "). Weights might be unstable or zero."))
  } else {
...
# .estimate_weighted_frechet_mean (MODIFIED for density optimization and OSQP caching)
.estimate_weighted_frechet_mean <- function(Y_obj_list, active_idx, s_in_weights_active,
  ...
  if (use_fallback_unweighted_mean) {
    ...
    if (metric_space_type == "density") {
      ...
      l_hat_q_unprojected <- colMeans(qin_active_fallback, na.rm = TRUE)
    ...
    if (metric_space_type == "density") {
      ...
      if (metric %in% c("frobenius", "power")) {
        if (metric_space_type == "network" || (metric_space_type %in% c("covariance", "correlation") && metric == "frobenius") || (metric_space_type %in% c("covariance", "correlation") && alpha == 1) ) {
        ...
        SS <- mean_L + diag(exp(mean_D_log_diag)); l_hat_mat_unproj <- t(SS) %*% SS
      } else if (metric == "cholesky") {
        # ... (cholesky logic as before) ...
      ...
      if (any(is.na(y0_unnorm)) || norm_y0_unnorm < 1e-9) {
        y0 <- yin_active_mat[1,]; if(is.null(y0)) { return(NULL) }
      } else { y0 <- y0_unnorm / norm_y0_unnorm }
      objFctn_sphere = function(y_sphere){
        ...
        f_val <- sum(s_in_weights_active * dists_sq_vals, na.rm=TRUE)
        g_val_terms <- matrix(0, nrow=nrow(yin_active_mat), ncol=ncol(yin_active_mat)); h_val_sum <- matrix(0, ncol=ncol(yin_active_mat), nrow=ncol(yin_active_mat))
        for(i_s in 1:nrow(yin_active_mat)) {
          dist_val <- frechet::SpheGeoDist(yin_active_mat[i_s,], y_sphere)
          if (abs(dist_val) > 1e-8 && abs(dist_val - pi) > 1e-8) {
            grad_i_val_unscaled <- frechet::SpheGeoGrad(yin_active_mat[i_s,], y_sphere); hess_i_val_unscaled <- frechet::SpheGeoHess(yin_active_mat[i_s,], y_sphere)
            g_val_terms[i_s,] <- s_in_weights_active[i_s] * 2 * dist_val * grad_i_val_unscaled
            h_val_sum <- h_val_sum + s_in_weights_active[i_s] * 2 * (grad_i_val_unscaled %*% t(grad_i_val_unscaled) + dist_val * hess_i_val_unscaled)
          }
        }
        return(list(value=sum(s_in_weights_active * dists_sq_vals, na.rm=TRUE), gradient=colSums(g_val_terms, na.rm=TRUE), hessian=h_val_sum))
      }
      res_trust <- try(trust::trust(objFctn_sphere, y0, rinit=0.1, rmax=100, iterlim=100), silent=TRUE)
      if (inherits(res_trust, "try-error") || !res_trust$converged) { l_hat_mean <- y0 } else { l_hat_mean <- res_trust$argument / frechet:::l2norm(res_trust$argument) }
    } else { stop(paste("Unsupported metric space type:", metric_space_type)) }
  }
  
  # Final projection/formatting step
  if (metric_space_type == "density") {
    # ... (density projection logic as before, with OSQP caching) ...
    m_dim <- length(l_hat_q_unprojected)
    cache_key_density <- paste("density_proj", m_dim, if (!is.null(frechet_optns$lower)) digest::digest(frechet_optns$lower) else "NULL_L", if (!is.null(frechet_optns$upper)) digest::digest(frechet_optns$upper) else "NULL_U")
    model_obj_density_proj <- NULL
    if (is.list(frechet_optns) && !is.null(frechet_optns$osqp_model_for_density_proj) && !is.null(frechet_optns$osqp_model_for_density_proj$key) && frechet_optns$osqp_model_for_density_proj$key == cache_key_density) {
      model_obj_density_proj <- frechet_optns$osqp_model_for_density_proj$model
    }
    if (is.null(model_obj_density_proj)) {
      Pmat_qp <- as(diag(m_dim), "sparseMatrix"); A_mono_i <- rep(1:(m_dim - 1), each = 2); A_mono_j <- unlist(lapply(1:(m_dim - 1), function(k) c(k, k + 1)))
      A_mono_vals <- rep(c(-1, 1), m_dim - 1); Amat_qp_mono <- Matrix::sparseMatrix(i = A_mono_i, j = A_mono_j, x = A_mono_vals, dims = c(m_dim - 1, m_dim))
      l_bounds_mono <- rep(0, m_dim - 1); u_bounds_mono <- rep(Inf, m_dim - 1); Final_Amat_qp <- Amat_qp_mono; Final_l_bounds_qp <- l_bounds_mono; Final_u_bounds_qp <- u_bounds_mono
      if (!is.null(frechet_optns$lower)) { Final_Amat_qp <- rbind(Final_Amat_qp, Matrix::sparseMatrix(i=1,j=1,x=1, dims=c(1,m_dim))); Final_l_bounds_qp <- c(Final_l_bounds_qp, frechet_optns$lower); Final_u_bounds_qp <- c(Final_u_bounds_qp, Inf) }
      if (!is.null(frechet_optns$upper)) { Final_Amat_qp <- rbind(Final_Amat_qp, Matrix::sparseMatrix(i=1,j=m_dim,x=1, dims=c(1,m_dim))); Final_l_bounds_qp <- c(Final_l_bounds_qp, -Inf); Final_u_bounds_qp <- c(Final_u_bounds_qp, frechet_optns$upper) }
      current_osqp_settings_density <- frechet_optns$osqp_settings_density_proj %||% frechet_optns$osqp_settings %||% osqp::osqpSettings(verbose = FALSE, eps_abs=1e-8, eps_rel=1e-8)
      q_placeholder_density <- rep(0, m_dim)
      model_obj_density_proj <- osqp::osqp(P=Pmat_qp, q = q_placeholder_density, A=Final_Amat_qp, l=Final_l_bounds_qp, u=Final_u_bounds_qp, pars = current_osqp_settings_density)
      if (is.list(frechet_optns)) { frechet_optns[['osqp_model_for_density_proj']] <- list(model = model_obj_density_proj, key = cache_key_density) }
    }
    model_obj_density_proj$Update(q = -l_hat_q_unprojected); osqp_sol <- model_obj_density_proj$Solve()
    l_hat_q_projected <- if (osqp_sol$info$status_val != 1 && osqp_sol$info$status_val != 2 ) { warning("OSQP for density projection failed."); l_hat_q_unprojected } else { osqp_sol$x }
    l_hat_mean <- list(q = l_hat_q_projected, qSup = frechet_optns$qSup, type="quantile")
    
  } else if (metric_space_type %in% c("covariance", "correlation", "network")) {
    # Determine if the network is directed from frechet_optns
    is_network_directed_local <- frechet_optns$network_directed %||% FALSE
    
    # Conditionally symmetrize for network space
    target_matrix_for_proj <- if (metric_space_type == "network" && is_network_directed_local) {
      l_hat_mat_unproj # Use potentially asymmetric for directed networks
    } else {
      (l_hat_mat_unproj + t(l_hat_mat_unproj)) / 2 # Symmetrize for undirected or cov/corr
    }
    
    if (metric_space_type == "network") {
      num_nodes_in_data <- ncol(Y_obj_list[[1]])
      ...
      l_hat_mean <- .project_to_laplacian_space_osqp(
        ...
        osqp_model_cache = frechet_optns$osqp_model_for_laplacian_proj, # This cache now includes directed_status
        osqp_settings = frechet_optns$osqp_settings_laplacian_proj,
        is_directed = is_network_directed_local # Pass the flag
      )
    } else { # Covariance / Correlation
      is_corr <- (metric_space_type == "correlation")
      # target_matrix_for_proj is already symmetrized for cov/corr
      l_hat_mat_pd <- try(Matrix::nearPD(target_matrix_for_proj, corr = is_corr, ensureSymmetry = TRUE, keepDiag = is_corr, maxit=100)$mat, silent = TRUE)
      if (inherits(l_hat_mat_pd, "try-error")) {
        warning(paste("nearPD failed for", metric_space_type, "mean. Using input to nearPD."))
        l_hat_mat <- target_matrix_for_proj
      } else {
        l_hat_mat <- as.matrix(l_hat_mat_pd)
      }
      if (is_corr) {
        diag(l_hat_mat) <- 1
        # Re-symmetrize after fixing diagonal for safety, though nearPD with ensureSymmetry should handle it.
...
# .estimate_one_sided_frechet_quantities (original function, ensure it calls modified .estimate_weighted_frechet_mean)
.estimate_one_sided_frechet_quantities <- function(Y_obj_list, X_scalar, metric_space_type,
  ...
  if (length(active_indices) == 0) {
    warning(paste("No data points have non-zero weight for side", side, ". Cannot estimate Frechet mean/variance."))
    return(list(l_hat = NULL, V_hat = NA, sigma_V_sq_hat = NA, num_active_weights = 0, debug_info = "No active weights"))
  }
  s_in_weights_active <- s_in_weights[active_indices]
  
  # This call now uses the modified .estimate_weighted_frechet_mean
  l_hat <- .estimate_weighted_frechet_mean(Y_obj_list, active_indices, s_in_weights_active,
                                           metric_space_type, frechet_optns) 
  if (is.null(l_hat)) {
    return(list(l_hat = NULL, V_hat = NA, sigma_V_sq_hat = NA, num_active_weights = length(active_indices), debug_info = "Frechet mean estimation failed"))
  }
  d_sq_vals_active <- sapply(active_indices, function(i_act) {
    dist_fun_sq_calculator(Y_obj_list[[i_act]], l_hat, frechet_optns)
  })
  
  # Variance estimation (using the corrected version from user's code)
  sum_w_sq_abs <- sum(abs(s_in_weights_active)) # Use absolute weights for the sum of squares part of variance
  sum_w_signed <- sum(s_in_weights_active)     # Use signed weights for the mean
  
  V_hat      <- sum(s_in_weights_active * d_sq_vals_active, na.rm=TRUE) / sum_w_signed  
  center     <- d_sq_vals_active - V_hat # d_sq_vals_active are deviations from this V_hat
  ...
  sigma_V_sq_hat <- if (sum_w_sq_abs > 1e-12) { # Avoid division by zero if all weights are tiny
    ...
    NA # Or some other indicator of failure / small sample
  }
  
  if (is.na(sigma_V_sq_hat) || sigma_V_sq_hat < 1e-12) { # Keep a floor
    sigma_V_sq_hat <- 1e-12
  }
  
  return(list(l_hat = l_hat, V_hat = V_hat, sigma_V_sq_hat = sigma_V_sq_hat, num_active_weights = length(active_indices), debug_info = NULL))
}

# .set_bw_range_frechet_style (original function - no changes)
.set_bw_range_frechet_style <- function(xin_side, c_val_target, kernel_type, X_scalar_global = NULL) {
  if (length(xin_side) < 2) {
    warning("Too few points to determine CV bandwidth range robustly for one side. Using wide heuristic.")
    sd_global_val <- NA
    if(!is.null(X_scalar_global)) sd_global_val <- stats::sd(X_scalar_global, na.rm=TRUE)
    if(is.na(sd_global_val) || sd_global_val == 0) sd_global_val <- 1
    min_bw <- 0.01 * sd_global_val
    max_bw <- 0.5 * sd_global_val
    return(list(min = max(1e-5, min_bw), max = max(max_bw, min_bw * 1.5)))
  }
  xinSt <- unique(sort(xin_side))
  ...
  if(xout_eff_min > xout_eff_max) {xout_eff_min <- min(xinSt); xout_eff_max <- max(xinSt)}
  min_spacing <- if(length(xinSt) > 1) min(diff(xinSt)) else 0.001
  bw_min_heuristic <- min_spacing * 1.5
  if(is.na(bw_min_heuristic) || bw_min_heuristic == 0) bw_min_heuristic <- 0.001
  kernel_scale_factor <- (ifelse(tolower(kernel_type) == "gauss", 3, 1) *
                            ifelse(tolower(kernel_type) == "gausvar", 2.5, 1))
  bw.min <- max(1e-5, bw_min_heuristic / kernel_scale_factor)
  ...
  if(is.na(range_xin_side) || range_xin_side == 0) {
    if(!is.na(sd_xin_side) && sd_xin_side > 0) range_xin_side <- sd_xin_side * 2
    else {
      if(!is.null(X_scalar_global)) {
        sd_global_val_inner <- stats::sd(X_scalar_global, na.rm=TRUE)
        if(!is.na(sd_global_val_inner) && sd_global_val_inner > 0) range_xin_side <- sd_global_val_inner * 2
        else range_xin_side <- 1
      } else {
        range_xin_side <- 1
      }
    }
  }
  if(is.na(range_xin_side) || range_xin_side == 0) range_xin_side <- 1
  bw.max <- max(bw.min * 1.5, range_xin_side / 3, na.rm=TRUE)
  if (bw.max < bw.min) {
    if (bw.min > bw.max * 1.5 && is.finite(bw.min) && is.finite(bw.max)) {
      bw.max <- bw.min * 1.01
    } else {
      bw.max <- (bw.min %||% 0.001) * 1.5
    }
  }
  bw.min <- if(is.finite(bw.min)) bw.min else 1e-5
  bw.max <- if(is.finite(bw.max)) bw.max else bw.min * 2
  if (bw.max <= bw.min) bw.max <- bw.min * 1.5
  return(list(min = bw.min, max = bw.max))
}

# ._cv_for_frechet_jump_h (original function, ensure it calls modified .estimate_weighted_frechet_mean)
._cv_for_frechet_jump_h <- function(Y_obj_list, X_scalar, c_val,
                                    metric_space_type, kernel_frechet_char,
                                    actual_kernel_function,
                                    frechet_optns, dist_fun_sq_calculator,
                                    K_folds = 5, n_bw_candidates = 10,
                                    min_eff_points_cv_fold = 3,
                                    min_bw_heuristic_factor = 0.001) { 
  
  n_total <- length(X_scalar)
  X_scalar_range <- diff(range(X_scalar, na.rm = TRUE))
  if(!is.finite(X_scalar_range) || X_scalar_range == 0) X_scalar_range <- 1 
  ...
  min_points_for_side_in_cv <- max(K_folds, ceiling(min_eff_points_cv_fold * K_folds / (K_folds - 1)) , 5)
  ...
  if (n_on_left_side < min_points_for_side_in_cv || n_on_right_side < min_points_for_side_in_cv) {
    ...
    if (length(X_around_c) < 2) {
      sd_X_global_fallback <- stats::sd(X_scalar, na.rm=TRUE); if(is.na(sd_X_global_fallback) || sd_X_global_fallback == 0) sd_X_global_fallback <-1
    ...
    if(is.na(h_heuristic) || h_heuristic <= 0) {
      ...
      valid_heur <- c(h_heur_left, h_heur_right)[is.finite(c(h_heur_left, h_heur_right)) & (c(h_heur_left, h_heur_right) > 0)]
  ...
  bw_range_right <- .set_bw_range_frechet_style(xin_side = X_scalar[indices_right_all],
                                                ...
                                                kernel_type = kernel_frechet_char,
                                                X_scalar_global = X_scalar)
  min_h_cand <- max(bw_range_left$min, bw_range_right$min, min_bw_heuristic_factor * X_scalar_range, na.rm = TRUE) 
  ...
  if (!is.finite(min_h_cand) || !is.finite(max_h_cand) || min_h_cand >= max_h_cand) {
    warning(paste("Combined CV: Invalid bandwidth range derived (min:",
                  signif(min_h_cand,3), "max:", signif(max_h_cand,3), "). Using global fallback range."))
    sd_X_global_fallback <- stats::sd(X_scalar, na.rm=TRUE); if(is.na(sd_X_global_fallback) || sd_X_global_fallback == 0) sd_X_global_fallback <-1
    min_h_cand <- max(1e-5, 0.05 * sd_X_global_fallback, min_bw_heuristic_factor * X_scalar_range)
    max_h_cand <- max(min_h_cand * 1.5, 0.5 * sd_X_global_fallback)
  }
  
  bw_candidates <- seq(min_h_cand, max_h_cand, length.out = n_bw_candidates)
  bw_candidates <- unique(pmax(1e-5, bw_candidates))
  
  shuffled_indices_left_within_side <- sample(1:n_on_left_side)
  folds_left <- cut(shuffled_indices_left_within_side, breaks = K_folds, labels = FALSE)
  shuffled_indices_right_within_side <- sample(1:n_on_right_side)
  ...
  cv_errors <- sapply(bw_candidates, function(h_cand) {
    fold_errors_h_sum <- 0
    num_valid_folds_for_h_cand <- 0
    
    suppressWarnings({ # Suppress warnings from inner estimations during CV
      for (k_fold_idx in 1:K_folds) {
        ...
        if (length(original_indices_train_left_fold) > 0) {
          s_in_train_weights_left <- .calculate_one_sided_locpoly_weights(
            x_obs = X_scalar[original_indices_train_left_fold], # Pass subset of X_scalar
            c_val = c_val, h_val = h_cand, K_fun = actual_kernel_function, side = "-", 
            n = length(original_indices_train_left_fold) # n for local poly is size of this subset
          )
          ...
          if (length(active_train_left_indices_in_subset) >= min_eff_points_cv_fold) {
            # original_indices_actively_weighted_train_left <- original_indices_train_left_fold[active_train_left_indices_in_subset] # Not directly needed if passing subset of Y_obj
            l_hat_train_left_at_c <- .estimate_weighted_frechet_mean(
              ...
              metric_space_type = metric_space_type, frechet_optns = frechet_optns
            )
          }
          if (is.null(l_hat_train_left_at_c)) { next }
        }
        ...
        if (length(original_indices_train_right_fold) > 0) {
          s_in_train_weights_right <- .calculate_one_sided_locpoly_weights(
            ...
            c_val = c_val, h_val = h_cand, K_fun = actual_kernel_function, side = "+", 
            n = length(original_indices_train_right_fold) # n for local poly is size of this subset
          )
          ...
          if (length(active_train_right_indices_in_subset) >= min_eff_points_cv_fold) {
            l_hat_train_right_at_c <- .estimate_weighted_frechet_mean(
              ...
              active_idx = active_train_right_indices_in_subset, # Indices relative to this Y_obj_list subset
              s_in_weights_active = s_in_train_weights_right[active_train_right_indices_in_subset],
              metric_space_type = metric_space_type, frechet_optns = frechet_optns
            )
          }
          if (is.null(l_hat_train_right_at_c)) { next }
        }
    ...
    if (num_valid_folds_for_h_cand == 0) return(Inf)
    return(fold_errors_h_sum / num_valid_folds_for_h_cand)
  })
  
  best_bw_idx <- which.min(cv_errors)
  if (length(best_bw_idx) == 0 || !is.finite(cv_errors[best_bw_idx[1]]) || cv_errors[best_bw_idx[1]] == Inf) {
    warning(paste("Combined K-Fold CV failed to find a valid minimum error. Min error found:",
                  if(length(best_bw_idx)>0 && length(cv_errors) >= best_bw_idx[1]) signif(cv_errors[best_bw_idx[1]],3) else "NA",
  ...
  return(bw_candidates[best_bw_idx[1]])
}